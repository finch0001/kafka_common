...
import com.intest.kafka.common.error.ZooKeeperClientException;
 ...
 * Kafka's new AdminClient
 */
public class CommonAdminClient implements Closeable {

    /**
     * Reference to the logger for this resource
     */
    private static final Logger LOG = LogManager.getLogger(CommonAdminClient.class);
     ...
     * </ul>
     */
    public CommonAdminClient(Properties properties) {
        if (properties == null) {
    ...
    public Properties getTopicConfig(String topic) {
        ...
        try {
            ...
            config.entries().stream().forEach(configEntry -> properties.setProperty(configEntry.name(),configEntry.value()));
    ...
    public static <T> scala.collection.immutable.Set<T> toImmutableScalaSet(Set<T> set) {
        ListBuffer<T> buffer = new ListBuffer<>();
        set.forEach((e) -> buffer.$plus$eq(e));

        // return Set$.<T>MODULE$.apply(buffer);
        return (scala.collection.immutable.Set<T>)Set$.MODULE$.apply(buffer);
    }
